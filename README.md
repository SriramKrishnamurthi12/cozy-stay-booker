...
Simple hotel booking app: add rooms, book a room for a date, view booking history. Prevents double-booking both in UI and backend.
...
Vite proxies `/api/*` to `http://localhost:4000`, so no extra config needed.
...
### HTTP status codes
- `400` validation error (response includes `fields` map)
- `404` room not found
...
2. **API**: Explicit pre-insert check in the booking controller returning `409` with message `Room X is already booked on YYYY-MM-DD`.
...
- Email validated by format only (no verification sent).
